...
import { ProjectJSON } from '../model/interfaces/project.model';
...
export class ProjectApiService extends ApiService {
  ...
  getProject(project_id: number): Observable<Project> {
    return this.http
      .get<ProjectJSON>(this.REST_URL + '/projects/' + project_id)
  ...
  getProjectsMatchTopic(topic: string): Observable<Project[]> {
    return this.getAllRessource(
      this.REST_URL + '/projects?topic=' + topic,
  ...
  getProjectsMatchSearch(search: string): Observable<Project[]> {
    return this.getAllRessource(
      this.REST_URL + '/projects?search=' + search + '&per_page=20',
    ).pipe(map((response) => response.map((item) => new Project(item))));
  }
  ...
  getProjectUsers(project_id: number): Observable<User[]> {
    return this.http
      .get<UserJSON[]>(this.REST_URL + '/projects/' + project_id + '/users')
  ...
  getProjectGroups(project_id: number): Observable<Group[]> {
    return this.http
      .get<GroupJSON[]>(this.REST_URL + '/projects/' + project_id + '/groups')
  ...
  getReadmeProject(project_id: number): Observable<string> {
    return this.http
      .get<any[]>(`${this.REST_URL}/projects/${project_id}/repository/tree`)
      .pipe(
        switchMap((items) => {
   ...
   */
  getRootProjectsIdMathSearch(search: string, cursor: string): Observable<any> {
