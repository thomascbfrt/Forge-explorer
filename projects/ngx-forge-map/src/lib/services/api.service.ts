...
export abstract class ApiService {
  /** Url for API request, can be change for a owser gitlab depot. */
  public GITLAB_URL: string = 'https://forge.apps.education.fr';

  /** Base URL for the Forge REST API endpoints. */
  protected readonly REST_URL = this.GITLAB_URL + '/api/v4';

  /** URL for the Forge GraphQL endpoint. */
  protected readonly graphql_url = this.GITLAB_URL + '/api/graphql';
  ...
  private getPage(
    url: string,
    page: number,
  ): Observable<{ body: any[]; nextPage: number | null }> {
    return this.http
