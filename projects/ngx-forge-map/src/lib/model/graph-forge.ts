...
import { ProjectJSON } from './interfaces/project.model';
import { TopicJSON } from './interfaces/topic.model';
import { GroupJSON } from './interfaces/group.model';
...
enum NodeImage {
  PROJECT = 'icons/icon-project/icon-project',
  USER = 'icons/icon-user/icon-user',
  GROUP = 'icons/icon-group/icon-group',
  TOPIC = 'icons/icon-topic/icon-topic',
}
...
export class GraphForge {
  ...
  private readonly reverse_ref_node = new Map<
    string,
    { type: NodeShape; id: number }
  >();
  ...
  createNode(
    ...
    node.color = {
      highlight: { border: '#ab3434', background: '#D2E5FF' },
      hover: { border: '#999999', background: '#D2E5FF' },
    };
  ...
  getNodesIDByType(type: number): number[] {
    ...
    nodes = nodes.filter(
      (n) => (n.id?.toString().split('==')[1] as unknown as number) == type,
    );

    return nodes.map(
      (n) => n.id!.toString().split('==')[2] as unknown as number,
    );
  }
  ...
  focus(id: string): void {
    this._network.focus(id, {
      animation: { duration: 2000, easingFunction: 'easeOutCubic' },
    });
  }
   ...
   */
  public createProject(project: ProjectJSON, size?: number): string {
    return this.createNode(
      project.name,
      NodeType.PROJECT,
      NodeShape.SQUARE,
      `${NodeImage.PROJECT}-selected.svg`,
      `${NodeImage.PROJECT}-unselected.svg`,
      project,
      size,
    );
  }
  ...
  /**
   * Creates a TOPIC node using the visual conventions and
   * stores the Topic model as the node payload.
   *
   * Wrapper around createNode with NodeType.TOPIC and default visual parameters.
   *
   * @param topic - Topic object containing at minimum an id and a name.
   * @param size - Optional size modifier.
   * @returns The internal identifier of the created topic node.
   */
  public createTopic(topic: TopicJSON, size?: number): string {
    return this.createNode(
      topic.name,
      NodeType.TOPIC,
      NodeShape.HEXAGON,
      `${NodeImage.TOPIC}-selected.svg`,
      `${NodeImage.TOPIC}-unselected.svg`,
      topic,
      size,
    );
  }
   ...
   */
  public createGroup(group: GroupJSON, size?: number): string {
    return this.createNode(
      group.name,
      NodeType.GROUP,
      NodeShape.TRIANGLE,
      `${NodeImage.GROUP}-selected.svg`,
      `${NodeImage.GROUP}-unselected.svg`,
      group,
      size,
    );
  }
   ...
   */
  public createUser(user: UserJSON, size?: number): string {
    return this.createNode(
      user.name,
      NodeType.USER,
      NodeShape.DOT,
      `${NodeImage.USER}-selected.svg`,
      `${NodeImage.USER}-unselected.svg`,
      user,
      size,
    );
  }
